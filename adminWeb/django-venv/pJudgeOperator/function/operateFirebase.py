...
class operateFirebase():
    def __init__(self):
        JSON_PATH = '/home/sakuTech/sakutech.pythonanywhere.com/adminWeb/django-venv/pJudgeOperator/function/saitenj-408a1-firebase-adminsdk-r2bv3-1e3090988e.json'
        # Firebase初期化
    ...
    def fWriteMemberDataToFirebase(self, engName, mid, startingMember, subMember):
        ...
        # documentに追加するときはset
        base_ref = self.db.collection(DB_BASE).document(engName).collection('Match').document(mid).collection('Member')
    ...
    def fReadMemberDataFromFirebase(self, teamName):
        ...
        doc_ref = self.db.collection(DB_BASE).document(teamName).collection('Member')
        docs = doc_ref.stream()
        for doc in docs:
            personalData = {
                'id':doc.id,
    ...
    def fReadMatchDataFromFirebase(self, teamName):
        ...
        doc_ref = self.db.collection(DB_BASE).document(teamName).collection('Match')
        docs = doc_ref.stream()
        for doc in docs:
            matchData = {
                'id':doc.id,
    ...
    def fReadPointsFromFirebase(self, teamName, matchID):
        ...
        doc_ref = self.db.collection(DB_BASE).document('Scores').collection(teamName).where('MatchID', '==', matchID)
        docs = doc_ref.stream()
        for doc in docs:
            appendData = [
                doc.get('MemberID'),
